...
async function calculateTrustScore(candidateData: any) {
  ...
  if (candidateData.totalRequests === 0) {
    ...
    const onlinePresenceScore = candidateData.onlinePresence
      ? candidateData.onlinePresence.combined_score
      : candidateData.socialProfiles.github ||
          candidateData.socialProfiles.linkedin
        ? 40
        : 20;
    ...
    return {
      ...
      breakdown: {
        ...
        online_presence: onlinePresenceScore,
        response_quality: 70,
        timeline_consistency: 80,
      },
      recommendations: [
        "Send verification requests to employers",
        ...(candidateData.onlinePresence
          ? []
          : ["Run online presence analysis for better scoring"]),
      ],
      risk_factors: ["No verification data available"],
      strengths: candidateData.onlinePresence
        ? candidateData.onlinePresence.agent_response?.strengths || []
        : [],
    };
  }
  ...
  const onlinePresenceInfo = candidateData.onlinePresence
    ? `Online Presence Analysis (via AgentVerse AI Agent):
...
function calculateFallbackScore(candidateData: any) {
  ...
  return {
    score: fallbackScore,
    analysis: `Fallback trust score based on ${(verificationRate * 100).toFixed(1)}% verification success rate. ${candidateData.successfulVerifications} out of ${candidateData.totalRequests} verifications completed successfully.${candidateData.onlinePresence ? " Online presence analysis included." : " Online presence analysis not available - consider running it for better scoring."}`,
    breakdown: {
    ...
    recommendations: [
      "Complete more verifications for accurate scoring",
      ...(candidateData.onlinePresence
        ? []
        : ["Run online presence analysis for better scoring"]),
    ],
    ...
    strengths: [
      ...(verificationRate > 0.8 ? ["High verification rate"] : []),
      ...(candidateData.onlinePresence
        ? candidateData.onlinePresence.agent_response?.strengths || []
        : []),
    ],
