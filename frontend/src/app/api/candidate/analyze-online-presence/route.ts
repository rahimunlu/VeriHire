...
const AGENTVERSE_API_URL = process.env.AGENTVERSE_API_URL || "https://agentverse.ai/v1/submit";
const AGENTVERSE_API_KEY = process.env.AGENTVERSE_API_KEY;
const AGENT_ADDRESS = "agent1qd9zadpfnxr28t7nmnr7nmtzlcs3kglyvslkagr7hzr6t64vrrpus036ads";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!github_profile && !linkedin_profile) {
      return NextResponse.json(
        { error: "No social profiles found. Please add GitHub or LinkedIn profile first." },
        { status: 400 }
      );
    }
    ...
    try {
      agentAnalysis = await callAgentVerseAgent(candidateId, github_profile, linkedin_profile, resume_data);
...
async function callAgentVerseAgent(candidateId: string, githubProfile: string, linkedinProfile: string, resumeData: any) {
...
Return analysis in JSON format with:
{
  ...
  const response = await fetch(AGENTVERSE_API_URL, {
    ...
    body: JSON.stringify({
      ...
      schema_digest: "proto:a90b71c8142c6b6c317d7297ef67015e90000361bb73b86eb9c70fb7a5ce1ce5",
  ...
  if (!response.ok) {
    throw new Error(`AgentVerse API error: ${response.status} ${response.statusText}`);
  }

  const agentResponse = await response.json();
  console.log("AgentVerse raw response:", JSON.stringify(agentResponse, null, 2));

  // Parse the agent response
  try {
    const analysisContent = agentResponse.response || agentResponse.payload || agentResponse;
...
function createFallbackAnalysis(githubProfile: string, linkedinProfile: string) {
  return {
    combined_score: 50,
    analysis_summary: "Fallback analysis - AgentVerse agent unavailable. Basic profile presence detected.",
    ...
    github_analysis: githubProfile ? {
      ...
      code_quality_indicators: "Profile exists but detailed analysis unavailable",
    ...
    linkedin_analysis: linkedinProfile ? {
      ...
      endorsements_recommendations: "Profile exists but detailed analysis unavailable"
    } : {},
    strengths: [
